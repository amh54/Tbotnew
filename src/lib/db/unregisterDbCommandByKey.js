...
async function unregisterDbCommandByKey(key, options = {}) {
  ...
  if (
    ...
    console.warn(
      "DB command unregister skipped: invalid dbCommandMap"
    );
    return;
  }
  ...
  // Check if this deck still exists with a different ID
  // (name is in current scan)
  const deckMovedToNewId =
    currentDeckNames &&
    info.rowData?.name &&
    currentDeckNames.has(info.rowData.name);
  ...
  for (const [otherKey, otherInfo] of dbCommandMap.entries()) {
    if (
      otherKey !== key &&
      otherInfo.commandName === info.commandName
    ) {
  ...
  console.log(
    `DB command unregistered: ${info.commandName}${
      commandStillInUse
        ? " (command still in use by another ID)"
        : ""
    }`
  );

  // Only send delete notification if deck was truly deleted,
  // not just moved to a new ID
  const isDeck = key.includes("decks");

  const isManuallyDeleted =
    globalThis.manuallyDeletedDecks?.has(key);
  ...
  if (
    ...
    // Use hero-specific thread channel instead of
    // general notification channel
    const threadChannelId =
      heroDeckThreadMap[tableConfig.table] ||
      heroDeckThreadMap[info.rowData?.hero];
  ...
  if (isDeck && !deckMovedToNewId && db) {
    await updateDeckbuilderCounts(
      db,
      info.rowData?.creator,
      -1
    );
  }
}

async function updateDeckbuilderCounts(
  db,
  creator,
  delta
) {
  ...
  const deckbuilderNames =
    await resolveDeckbuilderNames(
      db,
      creatorValue
    );
  ...
  try {
    ...
    console.error(
      "Error updating deckbuilder counts:",
      error
    );
  }
}
