    ...
    try {
      ...
      const choices = [
        ...
        .filter(
          (name) =>
            !search ||
            name.toLowerCase().replaceAll(/\s+/g, "").startsWith(search),
        )
        .slice(0, 25);
    ...
    try {
      ...
      if (!rows || rows.length === 0) {
        return interaction.editReply(
          `No ${hero} deck found with the name "${deckName}".`,
        );
      }

      const deckData = rows[0];

      const deletionKey = `web_decks:${
        deckData.deckID ?? deckData.deckid ?? deckData.id ?? deckData.name
      }`;
      ...
      return interaction.editReply({
        content: "An error occurred while trying to delete the deck.",
      });
    }
  },
};
