...
async function sendDeckbuilderLog(client, row, addedBy) {
  const thread = await client.channels
    .fetch(DECKBUILDER_LOG_THREAD_ID)
    .catch(() => null);

  if (!thread) {
    throw new Error(
      `Unable to fetch deckbuilder log thread: ${DECKBUILDER_LOG_THREAD_ID}`,
    );
  }
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("color")
        .setDescription(
          "Embed color for this deckbuilder, like #87CEEB or Blue",
        )
        .setRequired(true),
    )
  ...
  async execute(interaction) {
    ...
    if (!deckbuilderName) {
      return interaction.editReply({
        content: "Deckbuilder name cannot be blank.",
      });
    }
    ...
    if (!isValidEmbedColor(color)) {
      return interaction.editReply({
        content:
          "Invalid color. Use a hex color like #87CEEB or a Discord color name like Blue, Purple, or White.",
      });
    }
    ...
    try {
      const existingResult = await db.query(
        `
        SELECT deckbuilder_name
        FROM "web_deckbuilders"
        WHERE LOWER(REPLACE(deckbuilder_name, ' ', '')) = $1
        LIMIT 1
        `,
      ...
      try {
        ...
        logMessage =
          " The deckbuilder was added, but I could not send the log message.";
      }
