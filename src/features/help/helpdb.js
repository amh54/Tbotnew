...
async function fetchDecksFromSide(db, side) {
  try {
    ...
    return rows.map((row) => {
      ...
      return {
        id: row.deckID ?? row.deck_id ?? row.id ?? null,

        name: row.name ?? row.deckID ?? row.deck_id ?? row.id ?? "Unnamed",
    ...
    console.error(`Error querying web_decks for ${side}:`, err);
...
function matchesCategory(deck, category) {
  ...
  if (category === "budget") {
    return deck.categoryNorm.includes("budget");
  }

  if (category === "ladder") {
    return deck.categoryNorm.includes("ladder");
  }

  if (category === "meme") {
    return deck.categoryNorm.includes("meme");
  }

  if (category === "aggro") {
    return deck.archetypeNorm.includes("aggro");
  }

  if (category === "combo") {
    return deck.archetypeNorm.includes("combo");
  }

  if (category === "control") {
    return deck.archetypeNorm.includes("control");
  }

  if (category === "midrange") {
    return deck.archetypeNorm.includes("midrange");
  }

  if (category === "tempo") {
    return deck.archetypeNorm.includes("tempo");
  }
  ...
  for (const category of CATEGORIES) {
    deckLists[`plant_${category}`] = plantDecks
      ...
      .sort((a, b) => {
        ...
        return heroCompare === 0 ? a.name.localeCompare(b.name) : heroCompare;
      });
...
function buildCategoryEmbeds(deckLists) {
  ...
  for (const category of CATEGORIES) {
    const plantDeckNames = deckLists[`plant_${category}`].map(
      (deck) =>
        `${deck.name.replaceAll(/\s+/g, "").toLowerCase()} (${deck.hero})`,
    );
